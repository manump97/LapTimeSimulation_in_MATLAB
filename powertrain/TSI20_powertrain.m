...
% Engine
% Initial values
    pwt.ni = [1000 1500 2500 3000 3500 4000 4500 5000 5500 6500 6800]; % Initial n values [rpm]
    pwt.torquei = [320 320 440 440 440 440 440 440 440 370 350]; % Initial torque values [Nm]
% Min and Max n [rpm]
pwt.min_n = min(pwt.ni); % Minimum engine angular velocity [rpm]
pwt.max_n = max(pwt.ni); % Maximum engine angular velocity [rpm]
% Engine curves
pwt.interp_coef = polyfit(pwt.ni,pwt.torquei,5); % Interpolation coefficients
...
% Gearbox
pwt.gear_i_r = [1.018 0.698 0.466 0.335 0.233 0.204];
pwt.diff = [3.44 3.44 3.44 3.44 3.44 3.44];
pwt.gear_r = pwt.gear_i_r.*pwt.diff;

% Fuel
pwt.fuel_cons_ratio = 10; % Fuel Consumption [L/km]
...
pwt.n_lim_change = 6500; % n gear change upshift [rpm]
